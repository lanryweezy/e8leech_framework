...
#         """
#         self.assertEqual(np.linalg.matrix_rank(self.leech.basis), 24)
...
#     #     self.assertEqual(len(self.leech.generate_leech_roots_type1()), 1104)
...
#         gram_matrix = np.dot(self.leech.basis, self.leech.basis.T)
...
#         for v in self.leech.basis:
#             self.assertEqual(int(np.dot(v, v)) % 2, 0)

#         # Check that the dot product of any two basis vectors is an integer.
#         for i in range(len(self.leech.basis)):
#             for j in range(i, len(self.leech.basis)):
#                 self.assertAlmostEqual(np.dot(self.leech.basis[i], self.leech.basis[j]) % 1, 0)
...
#         v = np.zeros(24)
#         self.assertTrue(self.leech.check_leech_congruence(v))

#         # We need a vector that is not in the Leech lattice.
#         # A random vector is unlikely to be in the Leech lattice.
#         v = np.random.rand(24)
#         self.assertFalse(self.leech.check_leech_congruence(v))
